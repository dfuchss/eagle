...
/**
 * Defines an entry of an {@link ILayer}. An entry defines a single exploration step and belongs to exactly one layer.
 *
...
public interface ILayerEntry extends Serializable {
     ...
     *
     * @return iff {@code true} this entry belongs to a layer with no further layer after it.
     */
    @JsonIgnore
    boolean isLeaf();

    /**
     * Get the path to the root entry. Will return [this, ..., root] as a list of layer entries.
     *
    ...
    /**
     * Get the direct children of this layer entry. Each child belongs to an {@link IHypothesesSelection} in the parent
     * layer entry.
     *
    ...
    /**
     * Get a readable state representation of the data structure before agent execution.<br>
     * Iff {@code null} the result will be ignored.
     *
     * @return a readable state representation
     */
    String getStateRepresentationBeforeExecution();
